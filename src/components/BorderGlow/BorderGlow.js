...
const BorderGlow = ({
  ...
  useEffect(() => {
    ...
    const stopA = animateValue({ duration: 500, onUpdate: v => card.style.setProperty('--edge-proximity', v) });
    const stopB = animateValue({
      ease: easeInCubic,
      duration: 1500,
      end: 50,
      onUpdate: v => card.style.setProperty('--cursor-angle', `${(angleEnd - angleStart) * (v / 100) + angleStart}deg`),
    });
    const stopC = animateValue({
      ease: easeOutCubic,
      delay: 1500,
      duration: 2250,
      start: 50,
